...
spec:
  ...
  template:
    spec:
      containers:
        - name: vote
          image: sysdiglabs/example-voting-app-vote:statsd-5
          resources:
            limits:
              memory: 128Mi
              cpu: 200m
...
spec:
  ...
  template:
    spec:
      containers:
        - name: result
          image: sysdiglabs/example-voting-app-result:metrics-3
          resources:
            limits:
              memory: 64Mi
              cpu: 200m
...
spec:
  ...
  template:
    spec:
      containers:
        - name: worker
          image: sysdiglabs/example-voting-app-worker:jmx-1
          resources:
            limits:
              memory: 128Mi
              cpu: 500m
...
spec:
  ...
  template:
    spec:
      containers:
        - name: voter
          image: sysdiglabs/example-voting-app-voter:0.1
          env:
            - name: VOTE
...
spec:
  ...
  template:
    spec:
      containers:
        - name: observer
          image: sysdiglabs/recurling:0.1
          env:
            - name: URL
...
kubectl create -f resourcequota.yaml --namespace example-voting-app
...
rm redis-deployment.yaml redis-service.yaml db-deployment.yaml db-service.yaml vote-deployment.yaml vote-service.yaml result-deployment.yaml result-service.yaml worker-deployment.yaml voter-deployment.yaml observer-deployment.yaml resourcequota.yaml
